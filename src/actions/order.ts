...
export const getOrderById = async (orderId: number) => {
	...
	const { data: order, error } = await supabase
		.from('orders')
		.select(
			'*, addresses(*), customers(full_name, email), order_items(quantity, price, variants(target, type, kg, products(name, images)))'
		)
		.eq('customer_id', customerId)
		.eq('id', orderId)
		.single();
	...
	return {
		...
		orderItems: order.order_items.map(item => ({
			quantity: item.quantity,
			price: item.price,
			target: item.variants?.target,
			type: item.variants?.type,
			kg: item.variants?.kg,

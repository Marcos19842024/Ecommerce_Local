...
// Función para formatear cantidades con letras
const formatearCantidadConLetras = (monto: string): string => {
  // Convertir a número (remover símbolos de moneda y comas)
  const numero = parseFloat(monto.replace(/[$,]/g, ''));
  ...
  const enLetras = numeroALetras(numero);
  return `${enLetras} pesos m.n.`;
};
...
export const PdfEmploymentContract = ({ data }: { data: ContractData }) => {
  // Formatear los salarios a letras
  const salarioDiarioLetras = formatearCantidadConLetras(data.salarioDiario);
  const salarioSemanalLetras = formatearCantidadConLetras(data.salarioSemanal);
    ...
    <Document>
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <Text style={styles.clauseContent}>
          ...
          (<Text style={styles.bold}>{salarioDiarioLetras}</Text>), haciendo un total <Text style={styles.bold}>SEMANAL</Text> de 
          <Text style={styles.bold}> {data.salarioSemanal}</Text> (<Text style={styles.bold}>{salarioSemanalLetras}</Text>)
        </Text>
      </Page>
          ...
          <View style={[styles.tableRow, {padding: 5}]}>
            <View style={styles.tableCell}>
              {data.actividades.map((actividad, index) => (
                <View style={styles.flexDirection} key={index}>
              ...
              <View style={styles.flexDirection}>
                <Text style={styles.bold}>
                  {String.fromCharCode(97 + data.actividades.length) + ') '}
                </Text>
                <Text style={styles.actividadItem}>
                  Estas actividades no son limitativas. 
                </Text>
              </View>
            </View>
          </View>
